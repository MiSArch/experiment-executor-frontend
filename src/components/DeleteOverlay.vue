  ...
  <div v-if="showDeleteOverlay"
    ...
    <div class="flex flex-col w-full max-w-screen-lg m-24  rounded-lg">
      <div class="flex flex-row gap-2 justify-center items-center  rounded-t-lg pt-4 pr-4">
        ...
        <button @click="deleteExperiment" :disabled="loading" class="btn-header !bg-red-900 hover:!bg-red-800" title="Delete Experiment">
        ...
        <button @click="deleteExperimentVersion" :disabled="loading" class="btn-header !bg-red-900 hover:!bg-red-800"
                title="Delete Experiment Version">{{ loading ? 'Deleting Experiment...' : 'Delete Experiment Version' }}
        </button>
        <button @click="showDeleteOverlay = false" class="btn-header" title="Abort Delete">&times;</button>
      </div>
    </div>
  </div>
</template>
